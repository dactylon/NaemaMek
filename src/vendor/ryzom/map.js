...
Ryzom.XY = {
  ...
  fromIngameToOutgame: function(x, y) {
    return this._project(
      x,
      y,
      this.ingame_coordinates,
      this.outgame_coordinates
    );
  },
  ...
  _getZones: function(x, y, zones) {
    ...
    // if (Ryzom.DEBUG)
    //   console.debug('(_getZones) return', matches, ', input was ', [
    //     x,
    //     y,
    //     zones,
    //   ]);
  ...
  _findClosest: function(x, y, zones) {
    ...
    for (var prop in zones) {
      ...
      if (zones.hasOwnProperty(prop)) {
        ...
        for (
          var i = 0, len = zone.segments.length;
          i < len && current.distance > 0;
          i++
        ) {
  ...
  _belongsTo: function(x, y, zones, closest) {
    ...
    // if (Ryzom.DEBUG)
    //   console.debug('(_belongsTo) enter', [x, y, zones, closest]);
    ...
    if (closest && (matches.length == 0 || matches[0] == 'grid')) {
      // if (Ryzom.DEBUG)
      //   console.debug('>> zone match not found, try to find closest zone');
    ...
    // if (Ryzom.DEBUG)
    //   console.debug('(_belongsTo) return', [x, y], 'matches', matches);
    return matches;
  },
  ...
  _project: function(x, y, src, dst) {
    ...
    if (!srczone) {
      // if (Ryzom.DEBUG)
      //   console.debug('ERROR: no match', [srczone, dstzone, zonename, regions]);
      return { x: 0, y: 0 };
    }
    ...
    //     '>> source',
    //     [x, y],
    //     [pLeft.toFixed(3), pTop.toFixed(3)],
    //     [zonename]
    //   );
...
(function() {
  ...
  var worldZones = {
    world: [[0, 14160], [14160, 0]],
    continent_fyros: [[2920, 3836], [7400, 636]],
    continent_matis: [[7760, 7872], [13680, 352]],
    continent_tryker: [[7716, 13664], [13956, 8224]],
    continent_zorai: [[188, 13800], [5788, 8840]],
    continent_nexus: [[8196, 7896], [10116, 5656]],
    continent_bagne: [[8392, 5340], [9512, 3740]],
    continent_route_gouffre: [[5792, 11504], [7712, 4144]],
    continent_terre: [[2372, 7680], [5252, 4960]],
    continent_sources: [[692, 8600], [1972, 7000]],
    cont_newbieland: [[0, 16380], [3200, 14300]],
    cont_kitiniere: [[5000, 15880], [6280, 14600]],
    place_matis_island_1: [[8500, 15880], [9780, 14600]],
    place_matis_island_2: [[12880, 15880], [13840, 14600]],
    grid: [[-108000, 47520], [0, 0]],
  };
  var serverZones = {
    grid: [[0, -47520], [108000, 0]],
    continent_fyros: [[15840, -27040], [20320, -23840]],
    continent_matis: [[320, -7840], [6240, -320]],
    continent_tryker: [[13760, -34880], [20000, -29440]],
    continent_zorai: [[6880, -5920], [12480, -960]],
    place_pyr: [[18400, -24720], [19040, -24240]],
    continent_bagne: [[480, -11360], [1600, -9760]],
    continent_route_gouffre: [[5440, -16960], [7360, -9600]],
    continent_sources: [[2560, -11360], [3840, -9760]],
    continent_terre: [[160, -15840], [3040, -13120]],
    continent_nexus: [[7840, -8320], [9760, -6080]],
    place_yrkanis: [[4640, -3680], [4800, -3200]],
    place_natae: [[3600, -3840], [3840, -3680]],
    place_davae: [[4160, -4240], [4320, -4000]],
    place_avalae: [[4800, -4480], [4960, -4320]],
    place_dyron: [[16480, -24800], [16720, -24480]],
    place_thesos: [[19520, -26400], [19760, -26080]],
    place_avendale: [[18000, -31200], [18240, -30960]],
    place_crystabell: [[17760, -32000], [18000, -31760]],
    place_fairhaven: [[16960, -33280], [17440, -32720]],
    place_windermeer: [[15440, -33120], [15760, -32880]],
    place_zora: [[8480, -3040], [8800, -2720]],
    place_hoi_cho: [[9440, -3680], [9680, -3360]],
    place_jen_lai: [[8640, -3840], [8960, -3520]],
    place_min_cho: [[9760, -4320], [10080, -4000]],
    place_matis_island_1: [[14080, -1600], [15360, -320]],
    place_matis_island_2: [[15680, -1600], [16640, -320]],
    cont_newbieland: [[8160, -12320], [11360, -10240]],
    cont_kitiniere: [[1760, -17440], [3040, -16160]],
  };
...
L.Control.MousePosition = L.Control.extend({
  ...
  _onMouseMove: function(e) {
    ...
    html +=
      '<br>Image @(zoom=10): x:' + Math.floor(p.x) + ', y:' + Math.floor(p.y);
